...
"""Web app for updating LRI property"""
...
render = web.template.render("/var/www/apps-test/ccss_update/templates/")
...
defaultEntity = "Pick a CCSS Entity"
ccssEntities = []
entities = [defaultEntity] + ccssEntities
...
defaultData = "Enter CCSS updates here"

UpdateForm = form.Form(
    form.Dropdown("type", sorted(types), value=defaultType, id="types"),
    form.Dropdown("entity", sorted(entities), value=defaultEntity, id="entities"),
    form.Dropdown("format", sorted(formats), value=defaultFormat, id="formats"),
    form.Textarea("property", rows=30, cols=90, value="", id="properties"),
...
class Update(object):
    def GET(self, name=None):
        web.debug("Update.GET")

        updateForm = UpdateForm()
        return render.form(updateForm)

    def POST(self, name=None):
        web.debug("Update.POST")

        updateForm = UpdateForm()
        if not updateForm.validates():
            return render.form(updateForm)
