...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:variable name="g_resourcesBase" select="'https://example.com/api/rest'"/>

    <!-- Main template for laying out the document. It's all just
         DocBook XML code until the "xsl:for-each" calls used to process
         the elements of the modified WADL input.-->
    <xsl:template match="/">
        <part xml:id="doc-api-resources-all"
            ...
            <!-- The preface provides information on what you can find in
                 this part of the documentation and how to use it. -->
            <xi:include href="preface.xml"/>

            <!-- This first chapter exists to ensure that the HTML
                 page "scl_rest_api_resources-all.html" is still created and
                 maintained, providing guidance when a reader lands there. 
                 This used to be all resources on one page, which was an 
                 important reference and/or bookmark for application developers.
                 Now it links to other chapters which have separated out
                 resources based on their primary purpose. -->
            <chapter xml:id="rest_api_resources-all">
                <title>&RESTAPI; Resource Details</title>
                <para>
                    <emphasis role="bold"><link linkend="rest_api_resources-system">System Resources</link></emphasis> - 
                    Resources used to handle system operations such as signing
                    in and retreiving information about the current user and
                    session.
                </para>
                <para>
                    <emphasis role="bold"><link linkend="rest_api_resources-data-v1.0">Data Resources, version 1.0</link></emphasis> -
                    Resources used to retreive education data or to create,
                    update, or delete that data.
                </para>
                <para>
                    <emphasis role="bold"><link linkend="rest_api_resources-data-v1.1">Data Resources, version 1.1</link></emphasis> -
                    Resources used to retreive education data or to create,
                    update, or delete that data.
                </para>
            </chapter>
            
            <!-- This is the system calls chapter with the resources used when 
                 performing basic system operations such as signing in and
                 determining administrative access. This information comes 
                 from DocBook content, even though a handful of them also
                 appear in the modified WADL file. -->
            <xi:include href="rest_api_resources-system.xml"/>
            
            <!-- These loops create the database interaction chapters, one for
                 each released version of the API. Each chapter includes the
                 resources used when reading data and performing CRUD operations
                 with the $PRODUCTABBR; database. This information comes only 
                 from the modified WADL file, adding some boilerplate text from
                 the templates in this XSLT. -->
            
            <!-- Resources for version 1.1 -->
            <chapter xml:id="rest_api_resources-data-v1.1">
                <title>&RESTAPI; version 1.1: Data Resources</title>
                <para>
                    These are the resources used when reading and performing
                    CRUD operations with the &PRODUCTABBR; database.
                </para>
                <!-- For each resource in the modified WADL file... -->
                <xsl:for-each select="application/resources/resource">
                    <xsl:sort select="@path" order="ascending"/>
                    <xsl:if test="(not(contains(@path,'system')))and(not(contains(@path,'search')))">
            ...
            <!-- Resources for version 1.0 -->
            <chapter xml:id="rest_api_resources-data-v1.0">
                <title>&RESTAPI; version 1.0: Data Resources</title>
                <para>
                    These are the resources used when reading and performing
                    CRUD operations with the &PRODUCTABBR; database.
                </para>
            ...
            <xi:include href="ch-examples-v1.1.xml"/>
            
            <xi:include href="ch-examples-v1.0.xml"/>
            
            <xi:include href="ch-syntaxes.xml"/>
        ...
        <xsl:variable name="fullResourcePath">
            <xsl:choose>
                <xsl:when test="starts-with($resourcePath,'/')">
                    <xsl:value-of select="concat($g_resourcesBase,$resourcePath)"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="concat($g_resourcesBase,'/',$resourcePath)"/>
                </xsl:otherwise>
...
"transcript": {
    ...
    <xsl:template name="getModelLink">
        ...
        <xsl:variable name="modelEntityFile">
            <xsl:value-of select="'ch-data_model-entities.html'"/>
        </xsl:variable>
    ...
    <xsl:template name="getExampleBlock">
        <xsl:param name="apiVersion"/>
        <xsl:param name="resourceId"/>
        <xsl:param name="examplesFilename" select="concat('ch-examples-',$apiVersion,'.xml')"/>
        <xsl:choose>
